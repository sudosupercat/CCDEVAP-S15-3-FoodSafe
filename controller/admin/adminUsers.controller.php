<?php
require '../../model/admin.model.php';

// USER MANAGEMENT -- EDIT USER
$districts = getDistricts($pdo);
if (isset($_GET['action']) && $_GET['action'] === 'edit' && isset($_GET['id'])) {
    $selectedUser = getUserByID($pdo, $_GET['id']);
}

if(isset($_POST['action']) && $_POST['action'] === 'edit') {
    editUser($pdo, $_POST['userID'], $_POST['email'], $_POST['firstName'], $_POST['lastName'], $_POST['districtID']);

    header('Location: /CCDEVAP-S15-3-FoodSafe/controller/admin/adminUsers.controller.php?toast=edit');
    exit();
}

// USER MANAGEMENT -- UPDATE STATUS
if (isset($_POST['action']) && $_POST['action'] === 'update' && isset($_POST['id'])) {

    updateStatus($pdo, $_POST['id']);

    header('Location: /CCDEVAP-S15-3-FoodSafe/controller/admin/adminUsers.controller.php?toast=update');
    exit();
}

// USER MANAGEMENT -- DELETE USER
if (isset($_POST['action']) && $_POST['action'] === 'delete' && isset($_POST['id'])) {

    deleteUser($pdo, $_POST['id']);

    header('Location: /CCDEVAP-S15-3-FoodSafe/controller/admin/adminUsers.controller.php?toast=delete');
    exit();
}

// USER MANAGEMENT -- READ
$users = getUsers($pdo);

require '../../view/admin/user-management.php';

?>